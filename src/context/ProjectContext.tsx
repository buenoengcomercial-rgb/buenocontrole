...
function mapProjectPurchase(r: any): ProjectPurchase {
  return { id: r.id, projectId: r.project_id, supplierId: r.supplier_id, materialId: r.material_id, date: r.date, invoiceNumber: r.invoice_number, quantity: Number(r.quantity || 1), unitPrice: Number(r.unit_price || 0), totalValue: Number(r.total_value), freightValue: Number(r.freight_value || 0), icmsValue: Number(r.icms_value || 0), description: r.description, notes: r.notes, paymentMethod: r.payment_method || '', installments: r.installments || 1, firstInstallmentDate: r.first_installment_date || null, installmentDates: Array.isArray(r.installment_dates) ? r.installment_dates : null, installmentValues: Array.isArray(r.installment_values) ? r.installment_values.map((v: any) => Number(v)) : null, freightPaymentDate: r.freight_payment_date || null, icmsPaymentDate: r.icms_payment_date || null, createdAt: r.created_at };
}
...
export function ProjectProvider({ children }: { children: React.ReactNode }) {
  ...
  const addProjectPurchase = useCallback(async (p: Omit<ProjectPurchase, 'id' | 'createdAt'>) => {
    const { data } = await supabase.from('project_purchases').insert({
      ...
      payment_method: p.paymentMethod || '', installments: p.installments || 1, first_installment_date: p.firstInstallmentDate || null, installment_dates: p.installmentDates || null, installment_values: p.installmentValues || null, freight_payment_date: p.freightPaymentDate || null, icms_payment_date: p.icmsPaymentDate || null,
    }).select().single();
