...
export function ProjectProvider({ children }: { children: React.ReactNode }) {
  ...
  const addProjectPurchase = useCallback(async (p: Omit<ProjectPurchase, 'id' | 'createdAt'>) => {
    const { data } = await supabase.from('project_purchases').insert({
      ...
      payment_method: p.paymentMethod || '', installments: p.installments || 1, first_installment_date: p.firstInstallmentDate || null, installment_dates: p.installmentDates || null, freight_payment_date: p.freightPaymentDate || null, icms_payment_date: p.icmsPaymentDate || null,
    }).select().single();
