...
export default function ASOPage() {
  ...
  const alerts = useMemo(() =>
    asos.filter(a => daysUntilExpiry(a.expiryDate) <= 30)
  ...
  const grouped = useMemo(() => {
    const map: Record<string, typeof asos> = {};
    asos.forEach(a => {
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {grouped.map(([empId, items]) => {
          ...
          const expiredCount = items.filter(a => daysUntilExpiry(a.expiryDate) <= 0).length;
          return (
                ...
                <CollapsibleTrigger asChild>
                  <button className="w-full flex items-center justify-between px-6 py-4 hover:bg-muted/50 transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs text-muted-foreground bg-muted px-2 py-0.5 rounded-full">{items.length} ASO(s)</span>
                      {expiredCount > 0 && (
                        ...
                        {items.map(a => {
                          ...
                          return (
                            <React.Fragment key={a.id}>
                              ...
                              <tr><td colSpan={5} className="px-6 py-2 bg-muted/30"><AttachedDocuments entityType="aso" entityId={a.id} /></td></tr>
                            </React.Fragment>
                          );
                        })}
                      </tbody>
        ...
        {grouped.length === 0 && (
          <div className="bg-card rounded-xl shadow-card p-12 text-center text-meta">Nenhum ASO registrado.</div>
        )}
      </div>
    </div>
  );
