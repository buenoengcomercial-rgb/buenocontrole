...
interface ComparisonGroup { id: string; code: string; description: string; date: string; project_id: string | null; }
...
export default function ComparativosPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (gRes.data) setGroups(gRes.data.map((g) => ({ id: g.id, code: g.code, description: g.description, date: g.date, project_id: g.project_id })));
  ...
  const addGroup = async (description: string, projectId: string | null) => {
    ...
    if (data) { setGroups((prev) => [{ id: data.id, code: data.code, description: data.description, date: data.date, project_id: data.project_id }, ...prev]); setSelectedId(data.id); }
  };
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] flex-col bg-background text-foreground">
      <header className="flex items-center gap-2 border-b border-border bg-card px-4 py-2">
        ...
        {selected && (<Button size="sm" className="h-7 gap-1.5 text-xs" onClick={() => setOrderDialogOpen(true)}><FileText className="h-3.5 w-3.5" /> Gerar Pedido de Compra</Button>)}
      </header>

      <div className="flex flex-1 overflow-hidden">
        <div className="w-[520px] flex-shrink-0 overflow-hidden">
          <ComparisonGroupList groups={groups} projects={projects} selectedId={selectedId} onSelect={setSelectedId} onAdd={addGroup} onRemove={removeGroup} />
        </div>
        <div className="flex-1 overflow-hidden">
          {selected ? <SuppliersPanel suppliers={suppliers} onAdd={addSupplier} onRemove={removeSupplier} /> : <div className="flex h-full items-center justify-center text-sm text-muted-foreground">Selecione um comparativo</div>}
        </div>
      </div>
        ...
        <Tabs defaultValue="fornecimentos" className="flex h-full flex-col">
          <div className="flex items-center border-t border-border bg-card px-3">
            <TabsList className="h-8 bg-transparent p-0">
              {selected && (
                <>
                  <TabsTrigger value="fornecimentos" className="gap-1.5 rounded-none border-b-2 border-transparent px-3 py-1.5 text-xs data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none"><Table className="h-3 w-3" /> Fornecimentos</TabsTrigger>
                  <TabsTrigger value="analise" className="gap-1.5 rounded-none border-b-2 border-transparent px-3 py-1.5 text-xs data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none"><BarChart3 className="h-3 w-3" /> Análise de Cotação</TabsTrigger>
                  <TabsTrigger value="historico" className="gap-1.5 rounded-none border-b-2 border-transparent px-3 py-1.5 text-xs data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:shadow-none"><History className="h-3 w-3" /> Histórico de Preços</TabsTrigger>
                </>
              )}
            </TabsList>
          </div>
          {selected ? (
            <>
              <TabsContent value="fornecimentos" className="mt-0 flex-1 overflow-hidden"><ItemsTable items={items} suppliers={suppliers} prices={prices} onAddItem={addItem} onRemoveItem={removeItem} onUpdatePrice={updatePrice} onImportItems={importItems} /></TabsContent>
              <TabsContent value="analise" className="mt-0 flex-1 overflow-hidden"><CotacaoAnalysis items={items} suppliers={suppliers} prices={prices} /></TabsContent>
              <TabsContent value="historico" className="mt-0 flex-1 overflow-hidden"><PriceHistoryPanel history={history} /></TabsContent>
            </>
          ) : <div className="flex-1 flex items-center justify-center text-sm text-muted-foreground">Selecione um comparativo para ver os fornecimentos</div>}
        </Tabs>
      </div>

      {selected && <PurchaseOrderDialog open={orderDialogOpen} onOpenChange={setOrderDialogOpen} items={items} suppliers={suppliers} prices={prices} groupCode={selected.code} obraName={obraName} />}
    </div>
  );
}
